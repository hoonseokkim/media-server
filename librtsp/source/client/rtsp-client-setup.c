...
static int rtsp_client_media_setup(struct rtsp_client_t* rtsp, int i)
{
    ...
    case RTSP_TRANSPORT_RTP_TCP:
		// 2023-07-14, hslim: ffplay 연동을 위한 코드 수정
        ...
        len = snprintf(transport, sizeof(transport), "Transport: RTP/AVP/TCP;unicast;interleaved=%d-%d;mode=record\r\n", rtsp->transport[i].interleaved1, rtsp->transport[i].interleaved2);
        break;
    ...
    len = snprintf(rtsp->req, sizeof(rtsp->req), sc_rtsp_setup, rtsp->media[i].uri, transport, rtsp->cseq++, session, rtsp->authenrization, USER_AGENT);
